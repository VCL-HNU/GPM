    ...
    def compute_scores_across_datasets(self):
        ...
        if np.inf in self.mean_iou_per_dataset or np.nan in self.mean_iou_per_dataset:
            ...
            self.mean_iou_all_datasets = np.average(
                [f for f in self.mean_iou_per_dataset if f is not None], weights=w
                )  # np.mean(self.mean_iou_per_dataset)
...
def get_miou(results, gt_seg_maps):
    ...
    # results, gt_seg_maps: list of numpy.ndarray(H, W)
    results = [result.detach().cpu().numpy() for result in results]
    gt_seg_maps = [gt_seg_map.detach().cpu().numpy() for gt_seg_map in gt_seg_maps]

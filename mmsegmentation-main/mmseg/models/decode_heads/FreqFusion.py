...
class FreqFusion(nn.Module):
    ...
    def forward(self, hr_feat, lr_feat, use_checkpoint=False):
        assert hr_feat.shape[-1] > lr_feat.shape[-1], (hr_feat.shape, lr_feat.shape)
        if use_checkpoint:
...
class LocalSimGuidedSampler(nn.Module):
    ...
    def sample(self, x, offset, scale=None):
        ...
        return grid_sample(x.reshape(B * self.groups, -1, x.size(-2), x.size(-1)), coords).view(B, -1, scale * H, scale * W)

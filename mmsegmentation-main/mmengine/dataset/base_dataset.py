...
class BaseDataset(Dataset):
    ...
    Args:
        ...
        indices (int, float or Sequence[int], optional): Support using a few
            data in annotation file to facilitate training/testing on a smaller
            dataset. An int value indicates the used items in the dataset, float
            indicates fraction of items, Sequence[int] indicates item indices.
    ...
    def _get_unserialized_subset(self, indices: Union[Sequence[int],
        ...
        Args:
            indices (int or Sequence[int]): If type of indices is int,
                ...
                information. If type of indices is float, the fraction of
                datasets will be used. If type of indices is Sequence,
                indices represents the target data information index which
                consist of subset data information.
        ...
        if isinstance(indices, int):
            ...
            # Return fraction of dataset according to given fraction.
            assert 0 <= indices <= 1, f'If is float, indices should ' \
                                      f'between 0 and 1. Got {indices}'
            sub_data_list = []
            use_len = int(len(self.data_list) * indices)
            dataset_indices = list(range(len(self.data_list)))
            random.seed(0)
            random.shuffle(dataset_indices)
            use_indices = dataset_indices[:use_len]
    ...
    def __len__(self) -> int:
        ...
        mm_transfer.setdefault('cfg', dict())
        dataset_len = mm_transfer.cfg.get('dataset_len', None)

        if self.serialize_data:
            real_len = len(self.data_address)
        else:
            real_len = len(self.data_list)
        ...
        if dataset_len is None:
            ...
            has_been_shuffled = getattr(self, 'has_been_shuffled', False)
            if not has_been_shuffled:
                random.seed(0)
                random.shuffle(self.data_list)
                self.has_been_shuffled = True
            return min(real_len, dataset_len)

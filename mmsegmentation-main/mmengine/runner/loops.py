...
from .AdversarialAttack import get_method
...
class EpochBasedTrainLoop(BaseLoop):
    ...
    Args:
        ...
        attack_type (Optional[str]): 'white' or 'black' attack, or None.
        ckpt (Optional[str]): ckpt for black attack.
        method (Optional[Dict]): attack method.
    """
    ...
    def __init__(
            ...
            method: Optional[Dict] = None) -> None:
        ...
        if attack_type is not None:
            assert attack_type in ['black', 'white'], attack_type
            if attack_type == 'black':
                assert ckpt is not None
                ckpt = torch.load(ckpt)
                self.white_model = copy.deepcopy(runner.model).load_state_dict(ckpt)
            elif attack_type == 'white':
                self.white_model = runner.model
            attack_method = method.pop('type')
            self.method = method
            self.attacker = get_method(attack_method, self.white_model, **self.method.init_cfg)
        else:
            self.attacker = None
    ...
    def run(self) -> torch.nn.Module:
        ...
        while self._epoch < self._max_epochs and not self.stop_training:
            ...
            if (self.runner.val_loop is not None
                ...
                # if self.runner.aa_val_loop is not None:
                #     self.runner.aa_val_loop.run()
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        if self.attacker is not None:
            data_batch = self.attacker(data_batch, **self.method.call_cfg)
...
class ValLoop(BaseLoop):
    ...
    def run(self) -> dict:
        ...
        if self.attacker is not None:
            metrics = dict(('AA_' + key, value) for key, value in metrics.items())
...
class TestLoop(BaseLoop):
    ...
    Args:
        ...
        dataloader (Dataloader or dict): A dataloader object or a dict to
            build a dataloader.
        evaluator (Evaluator or dict or list): Used for computing metrics.
        attack_type (str): 'white' or 'black' attack.
        ckpt (str): ckpt for black attack.
        method (Dict): attack method.

...
from mmengine.dist import (broadcast, get_dist_info, get_rank, get_world_size,
                           init_dist, is_distributed, master_only, is_main_process)
...
class Runner:
    ...
    def __init__(
        ...
        # lazy initialization
        self.can_be_trained = True
        if train_dataloader is None:
            self.can_be_trained = False
            train_cfg = None
            optim_wrapper = None
    ...
    @property
    def aa_val_loop(self):
        """:obj:`BaseLoop`: A loop to run adversarial-attack validation."""
        if self.val_loop is None:
            return None
        if hasattr(self, 'adversarial_val_enabled'):
            return self._aa_val_loop
        else:
            self.adversarial_val_enabled = False
            if 'adversarial_test' in self.cfg and self.cfg.adversarial_test is not None:
                adversarial_val = copy.deepcopy(self.cfg.adversarial_test)
                self.adversarial_val_enabled = True
            elif 'adversarial_train' in self.cfg and self.cfg.adversarial_train is not None:
                adversarial_val = copy.deepcopy(self.cfg.adversarial_train)
                self.adversarial_val_enabled = True
            if self.adversarial_val_enabled:
                adversarial_val.update(self.cfg.val_cfg)
                self._aa_val_loop = self.build_val_loop(adversarial_val)
            else:
                self._aa_val_loop = None
            return self._aa_val_loop
    ...
    def build_train_loop(self, loop: Union[BaseLoop, Dict]) -> BaseLoop:
        ...
        if 'adversarial_train' in self.cfg and self.cfg.adversarial_train is not None:
            adversarial_train = copy.deepcopy(self.cfg.adversarial_train)
            loop_cfg.update(adversarial_train)
    ...
    def build_test_loop(self, loop: Union[BaseLoop, Dict], dataloader=None) -> BaseLoop:
        """Build test loop.
        ...
        if 'type' in loop_cfg:
            loop = LOOPS.build(
                ...
                default_args=dict(
                    runner=self,
                    dataloader=self._test_dataloader if dataloader is None else dataloader,
    ...
    def load_or_resume(self) -> None:
        ...
        if resume_from is not None:
            self.has_resume = resume_from
            self.resume(resume_from)
            self._has_loaded = True
        elif self._load_from is not None:
            self.has_load_from = self._load_from
    ...
    def train(self) -> nn.Module:
        ...
        """
        if not self.can_be_trained:
            self.logger.warning('train_dataloader is set to None, hence '
    ...
    def test(self) -> dict:
        ...
        def clean_dataloader(dataloader):
            dataloader = copy.deepcopy(dataloader)
            pop_keys = [
                'pin_memory', 'persistent_workers', 'collate_fn', 'batch_size',
                'num_workers', 'sampler', 'pipeline'
            ]
            for pop_key in pop_keys:
                dataloader.pop(pop_key, None)
            return dataloader

        def do_test(test_cfg):
            self.call_hook('before_run')
            # make sure checkpoint-related hooks are triggered after `before_run`
            self.load_or_resume()
            metrics = self.test_loop.run()  # type: ignore
            self.call_hook('after_run')
            return 'test_cfg:\n{}metrics = {}\n\n\n'.format(Config(test_cfg).pretty_text, metrics)

        def test_datasets_and_loops(test_loops):
            test_loops = test_loops if isinstance(test_loops, list) else [test_loops]
            for dataloader in test_dataloaders:
                for test_loop in test_loops:
                    test_cfg = OrderedDict(
                        dataloader=clean_dataloader(dataloader),
                        test_loop=test_loop,
                    )
                    self._test_loop = self.build_test_loop(test_loop, dataloader)
                    test_results.append(do_test(test_cfg))
        ...
        _test_loop = self._test_loop
        if is_main_process():
            print('\nDoing test..')

        test_dataloaders = self._test_dataloader if isinstance(self._test_dataloader, list) else [self._test_dataloader]

        # test without adversary
        test_datasets_and_loops(
            test_loops=_test_loop
        )

        if 'adversarial_test' in self.cfg and self.cfg.adversarial_test:
            # test with adversary
            test_datasets_and_loops(
                test_loops=self.cfg.adversarial_test
            )
    ...
    def resume(self,
        ...
        # resume others
        if 'others' in checkpoint:
            mm_transfer.resumed = checkpoint['others']
        else:
            mm_transfer.resumed = dict()
        if not resume_optimizer:
            mm_transfer.resumed.pop('sdl_inner_count', None)
    ...
    def save_checkpoint(
        ...
        if save_optimizer:
            ...
            if 'sdl_inner_count' in mm_transfer:
                checkpoint['others']['sdl_inner_count'] = mm_transfer.sdl_inner_count

...
def parse_args():
    ...
    parser.add_argument(
        '--config',
        default="configs/GPM/faster-rcnn_r18_fpn-baseline.py",
...
def before_runner(args, cfg):
    def _initialize():
        ...
        # If auto_accumulative_counts is enabled, automatically set "accumulative_counts"
        # so that the real batch_size * accumulative_counts == base_batch_size
        auto_accumulative_counts = cfg.get('auto_accumulative_counts', False)
        if auto_accumulative_counts:
            base_batch_size = cfg.auto_scale_lr.base_batch_size
            assert base_batch_size % cfg.batch_size == 0
            cfg.optim_wrapper.accumulative_counts = base_batch_size // cfg.batch_size
            print(
                f'Automatically set "accumulative_counts" to {cfg.optim_wrapper.accumulative_counts} '
                f'based on batch_size = bs({cfg.batch_size}), and base_batch_size = {base_batch_size}'
                )

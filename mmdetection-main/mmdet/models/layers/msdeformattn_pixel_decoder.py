...
class FreqAwareMSDeformAttnPixelDecoder2(MSDeformAttnPixelDecoder):
    ...
    def __init__(self,
        ...
        conv_dim = kwargs.get('feat_channels', 256)
    ...
    def forward(self, feats: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        for idx, f in enumerate(encoder_input_list[:-1]):
        ...
        for idx, feat_projected in enumerate(encoder_input_list):
            encoder_input_list[idx] = feat_projected.flatten(2).permute(0, 2, 1)
        ...
        for i in range(self.num_input_levels - self.num_encoder_levels - 1, -1,
            ...
            pre_feat = outs[-1]
            _, cur_feat, pre_feat = self.freqfusions[-1](hr_feat=cur_feat, lr_feat=pre_feat, use_checkpoint=False)
            y = cur_feat + pre_feat

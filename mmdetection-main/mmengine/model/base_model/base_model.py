...
class BaseModel(BaseModule):
    ...
    def train_step(self, data: Union[dict, tuple, list],
        ...
        """
        # For calculation of second derivative loss

        # Enable automatic mixed precision training context.
        with optim_wrapper.optim_context(self):
            data = self.data_preprocessor(data, True)
            if 'type' in mm_transfer.cfg.optim_wrapper and mm_transfer.cfg.optim_wrapper.type == 'SAGMOptimWrapper':
                mm_transfer.data = data
            if mm_transfer.cfg.has_sd_loss:
                self.sdloss_preprocess(data)
            losses = self._run_forward(data, mode='loss')  # type: ignore
        parsed_losses, log_vars = self.parse_losses(losses)  # type: ignore
        optim_wrapper.update_params(parsed_losses)
        if mm_transfer.cfg.has_sd_loss:
            log_vars = self.parse_sd_losses(log_vars)
        return log_vars
    ...
    def sdloss_preprocess(self, data: Union[dict, tuple, list]):
        """
        preprocess for second derivative losses
        """

        data['inputs'].requires_grad = True
        mm_transfer.data = data
        mm_transfer.temp['fd_loss_used'] = dict()
    ...
    def parse_losses(
        ...
        if mm_transfer.cfg.has_sd_loss:
            loss_use = getattr(mm_transfer.cfg.sdloss_cfg, 'loss_use', ['all'])
            if loss_use != 'all' and loss_use != ['all']:
                loss_use = loss_use if isinstance(loss_use, list) else [loss_use]
                fd_loss_used = mm_transfer.temp['fd_loss_used'] if mm_transfer.temp['fd_loss_used'] else log_vars
                mm_transfer.temp['fd_loss_used'] = dict((key, value) for key, value in fd_loss_used.items() if 'loss' in key and key in loss_use)
            if  'fd_loss_used' in mm_transfer.temp and mm_transfer.temp['fd_loss_used']:
                mm_transfer.temp['fd_loss_used'] = sum(value for key, value in mm_transfer.temp['fd_loss_used'] if 'loss' in key)

        mm_transfer.fd_loss = loss.detach()
        fd_losses = []
        count = 1
        for key, value in log_vars:
            if 'loss' in key:
                if key != 'loss':
                    fd_losses.append((key, value.detach()))
                else:
                    fd_losses.append((f'fd_loss{count}', value.detach()))
                    count += 1
        mm_transfer.fd_losses = OrderedDict(fd_losses)
    ...
    def parse_sd_losses(
        ...
        """Parses second derivative losses.

        Args:
            log_vars (OrderedDict): second output of self.parse_losses.

        Returns:
            log_vars (OrderedDict):  which will be sent to the logger.
        """
        fd_loss = mm_transfer.fd_loss
        fd_loss_used = mm_transfer.temp['fd_loss_used'] if mm_transfer.temp['fd_loss_used'] else fd_loss
        fd_losses = mm_transfer.fd_losses
        sd_loss = mm_transfer.sd_loss
        sd_losses = mm_transfer.sd_losses

        losses = OrderedDict(loss=fd_loss + sd_loss)
        losses.update(fd_loss=fd_loss)
        losses.update(fd_loss_used=fd_loss_used)
        losses.update(fd_losses)
        losses.update(sd_loss=sd_loss)
        losses.update(sd_losses)
        losses = list(losses.items())

        for name, _ in losses:
            log_vars.pop(name, None)
        log_vars = list(log_vars.items())
        log_vars = losses + log_vars
        log_vars = OrderedDict(log_vars)

        mm_transfer.log_vars = log_vars
        return log_vars

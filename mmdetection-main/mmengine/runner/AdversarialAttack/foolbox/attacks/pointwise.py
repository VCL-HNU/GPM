...
class PointwiseAttack(FlexibleDistanceMinimizationAttack):
        ...
        **kwargs: Any,
        ...
        while ep.any(found_index_to_manipulate):
            ...
            while i < max([len(it) for it in untouched_indices]):
                ...
                relevant_mask: np.ndarray = np.array(
                    relevant_mask_lst, dtype=bool
                )
                relevant_mask_index: np.ndarray = np.flatnonzero(relevant_mask)
    ...
    def _binary_search(
        ...
        mask: Union[ep.Tensor, List[bool], np.ndarray],
        mask_indices: Union[ep.Tensor, np.ndarray],

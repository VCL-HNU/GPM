...
from .sdloss_calculate_and_backward import sdl_calculate_and_backward
...
class OptimWrapper(BaseOptimWrapper):
    ...
    def __init__(self,
        ...
        mm_transfer.setdefault('optim_wrappers', []).append(self)

        if clip_grad is not None:
            # clip_grad_kwargs should not be empty dict.
    ...
    def update_params(  # type: ignore
        ...
        if self.should_update():
            ...
            mm_transfer.record_grads(mm_transfer.runner.model)
    ...
    def backward(self, loss: torch.Tensor, **kwargs) -> None:
        ...
        if mm_transfer.cfg.has_sd_loss:
            sdl_calculate_and_backward(
                fd_loss=loss,
                inputs=mm_transfer.data['inputs'] if isinstance(mm_transfer.data, dict) else mm_transfer.data,
                loss_scaler=None,
            )
        else:
